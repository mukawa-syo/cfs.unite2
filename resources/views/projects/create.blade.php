...
@section('content')
    ...
    <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data" id="projectForm">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="project_description" name="project_description" rows="3" required>{{ old('project_description') }}</textarea>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control" id="target_amount" name="target_amount" value="{{ old('target_amount') }}" min="1" required>
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select" id="project_category_id" name="project_category_id" required>
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->project_category_id }}">{{ $category->category_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="initiator_name" name="initiator_name" value="{{ old('initiator_name') }}" required>
        </div>
            ...
            <div id="rewards">
                <div class="reward-item mb-4 p-4 border rounded">
                    <div class="form-group mb-3">
                        <label for="reward_name_0" class="form-label">リワード名</label>
                        <input type="text" class="form-control" name="rewards[0][reward_name]" id="reward_name_0" required>
                    </div>
                    <div class="form-group mb-3">
                        <label for="price_incl_tax_0" class="form-label">金額（税込）</label>
                        <input type="number" class="form-control" name="rewards[0][price_incl_tax]" id="price_incl_tax_0" required min="1">
                    </div>
                    <div class="form-group mb-3">
                        <label for="reward_description_0" class="form-label">リワードの説明</label>
                        <textarea class="form-control" name="rewards[0][reward_description]" id="reward_description_0" rows="3" required></textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label for="reward_image_0" class="form-label">リワード画像</label>
                        <input type="file" class="form-control" name="rewards[0][reward_image]" id="reward_image_0" accept="image/*">
                    </div>
                    <div class="form-group mb-3">
                        <label for="delivery_schedule_0" class="form-label">お届け予定日</label>
                        <input type="date" class="form-control" name="rewards[0][delivery_schedule]" id="delivery_schedule_0" required>
                    </div>
                </div>
            </div>
    ...
    addRewardButton.addEventListener('click', function() {
        ...
        const rewardTemplate = `
            <div class="reward-item mb-4 p-4 border rounded">
                ...
                <button type="button" class="btn btn-danger btn-sm remove-reward">このリワードを削除</button>
            </div>
        `;
        rewardsContainer.insertAdjacentHTML('beforeend', rewardTemplate);

        // 削除ボタンのイベントリスナーを追加
        const removeButtons = document.querySelectorAll('.remove-reward');
